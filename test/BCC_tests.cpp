...
TEST_F(BCC_TEST, BCC_RE_CarrySetNoBranch) {
    ...
    cpu.memory[0xFFFC] = BCC_REL_OPCODE;
    cpu.memory[0xFFFD] = 0x02; // Jump 2 bytes
    // End inline program

    int cycles = em6502_execute(&cpu, BCC_REL_CYCLES);

    ASSERT_EQ(cycles, BCC_REL_CYCLES);
...
TEST_F(BCC_TEST, BCC_RE_NoCarrySetBranchJumpTwoBytesAndToANewPage) {
    ...
    ASSERT_EQ(cycles, BCC_REL_CYCLES + 2);
...
TEST_F(BCC_TEST, BCC_RE_NoCarrySetBranchJumpOneByteNoNewPage) {
    ...
    cpu.memory[0xFFFE] = 0x00;          // skipped
    cpu.memory[0xFFFF] = 0x00;          // skipped
    cpu.memory[0x0000] = BCC_REL_OPCODE; // Run instruction again
    cpu.memory[0x0001] = 0x02;          // Jump 2 bytes
    cpu.memory[0x0002] = 0x00;          // skipped
    cpu.memory[0x0003] = 0x00;          // skipped
    cpu.memory[0x0004] = BCC_REL_OPCODE; // Next instruction
    // End inline program

    int cycles = em6502_execute(&cpu, (BCC_REL_CYCLES + 2) + BCC_REL_CYCLES);

    cycles -= BCC_REL_CYCLES +
              2; // Remove the cycles which are used to jump the PC to 0x0001

    ASSERT_EQ(cycles, BCC_REL_CYCLES + 1);
