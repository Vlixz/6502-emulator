...
TEST_F(BNE_TEST, BNE_TEST_ZeroFlagSetNoBranch) {
    ...
    cpu.memory[0xFFFC] = BNE_REL_OPCODE;
    cpu.memory[0xFFFD] = 0x02; // Jump 2 bytes
    // End inline program

    int cycles = em6502_execute(&cpu, BNE_REL_CYCLES);

    ASSERT_EQ(cycles, BNE_REL_CYCLES);
...
TEST_F(BNE_TEST, BNE_REL_NoZeroFlagSetBranchJumpTwoBytesAndToANewPage) {
    cpu.Z = 0;
    ...
    cpu.memory[0x0000] = BNE_REL_OPCODE; // next instruction
    // End inline program

    int cycles = em6502_execute(&cpu, BNE_REL_CYCLES);

    ASSERT_EQ(cycles, BNE_REL_CYCLES + 2);
...
TEST_F(BNE_TEST, BNE_REL_NoZeroFlagSetBranchJumpOneByteNoNewPage) {
    cpu.Z = 0;
    ...
    cpu.memory[0xFFFF] = 0x00; // skipped
    cpu.memory[0x0000] =
        BNE_REL_OPCODE; // Run instruction again without going to a new page
    cpu.memory[0x0001] = 0x02;          // Jump 2 bytes
    cpu.memory[0x0002] = 0x00;          // skipped
    cpu.memory[0x0003] = 0x00;          // skipped
    cpu.memory[0x0004] = BNE_REL_OPCODE; // Next instruction
    // End inline program

    int cycles = em6502_execute(&cpu, (BNE_REL_CYCLES + 2) + BNE_REL_CYCLES);

    cycles -= BNE_REL_CYCLES +
              2; // Remove the cycles which are used to jump the PC to 0x0001

    ASSERT_EQ(cycles, BNE_REL_CYCLES + 1);
